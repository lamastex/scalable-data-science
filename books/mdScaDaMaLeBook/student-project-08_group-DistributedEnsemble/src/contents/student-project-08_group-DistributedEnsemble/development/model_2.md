...
Not a big change, but it is nice to be able to create models from a list of paths to state dicts -- saved to disk -- without having to know the shapes a priori.

The important changes are: - added static methods in `MLP` class - `Train` function removes the `shapes` argument.
