...
// MAGIC %md 
// MAGIC 
// MAGIC ## Scalable web scrapper -- Based on MS4DS book (Chap. 6)
// MAGIC 
// MAGIC This cleaned up notebook will contain code to perform web scraping, which should then be called from another notebook

// COMMAND ----------

// MAGIC %md Need to install the goose web scraper library, available with maven coordinate:
// MAGIC 
...
// MAGIC %md The functions for expanding the urls
...
// MAGIC %md To get the articles from the web, use a dataframe of urls and return a dataframe with body, domain,title
...
// MAGIC %md flattening the dataframe that does not need information on the number of new columns we want (copied with modification from notebook 007 of the course)
...
// MAGIC %md finally a function to do all the above 
