...
- 1.5 : Reasonably organised and could be improved (please consider making suggestions for improvement in Section 8 below) for easier comprehension.
...
- Got stuck at the step of data-reformatting when trying to run the project. The dataset is not small to download and it seems that I need to download all of json files to run it or I have to modify the code. Perhaps just use one slice of the dataset for the reviewer to try out. 
- Overall, the repository is quite well-organized and the amount of code is manageable.
