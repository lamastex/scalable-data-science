...
# MAGIC %md
# MAGIC Loading and transforming the dataset.
...
# MAGIC %md
# MAGIC Running and loading the functions to create and save the transformed data.
...
# MAGIC %md
# MAGIC Let's also plot the model architecture to verify that we got the [ICNet](https://arxiv.org/pdf/1704.08545.pdf) architecture correctly. We first save the model in `png` format on DBFS through the package `tf.keras.utils.plot_model` and then load and display it through `matplotlib.image` package.
...
# MAGIC %md
# MAGIC Compiling the model with optimizer *AdamW* (*Adam* with `weight_decay`), loss function *SparseCategoricalCrossentropy* and metrics *SparseCategoricalAccuracy*. We also add loss weights 0.4, 0.4, 1 and 0 to the lower resolution output, medium resolution output and high resolution output and final output (only evaluated in testing phase) respectively. 
...
# MAGIC %md
# MAGIC Now we define the batch generator which will be passed to `model.fit()` function.
...
# MAGIC %md
# MAGIC We create a callback for early stopping to prevent overfitting.
...
# MAGIC %md
# MAGIC MLFlow is initialized to keep track of the experiments.
...
# MAGIC %md
# MAGIC We visualize the accuracies and losses through the library `matplotlib`. This can also be seen in the MLFlow experiment visible under *Experiments* in the top right corner of the notebook.
