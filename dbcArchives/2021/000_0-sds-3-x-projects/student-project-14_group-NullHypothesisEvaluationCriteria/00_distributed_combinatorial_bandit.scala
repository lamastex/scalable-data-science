...
// DBTITLE 1,Separate the list over nodes to pairs
...
// DBTITLE 1,Select only ways that can be driven on, and fill resonable maxspeed when unavailable.
...
// DBTITLE 1,Calculate links and filter
import org.apache.spark.sql.functions._
...
// DBTITLE 1,Make graph from edges
...
// MAGIC %md
// MAGIC ## The shortest path algorithm
// MAGIC The implemented shorthest path alborithm uses the the distributed Pregel algorithm and is divided into two parts.
// MAGIC 
// MAGIC The first part is based on the code in <http://lamastex.org/lmse/mep/src/GraphXShortestWeightedPaths.html>. As the original code did not have have all functionality desiered functionality, 
// MAGIC the algorithm did find the shortest distance but didn't keep track of the path itself,  the algorithm was extend with this functionality.
// MAGIC 
// MAGIC The first part takes a graph, where the edges are double values representing the cost of trevelling between its connected nodes,  and an array of the ids of each goal node.
...
// MAGIC 
// MAGIC The second part transforms the output of the first part to a "path graph" where each edge is marked with either a 1 or a 0 depending on if it is used in a path between a starting node and a goal node. Altough this recursion can be performed on a single machine for small examples, this procedure is also implemented using the Pregel algorithm to handle situations of millions of edges.
// MAGIC 
// MAGIC The input of the second part is the graph created in the first part as well as the id of a single goal node and a start node. The goal node has to be in the set of goal nodes used in the first part.
// MAGIC This part outputs a "path graph" where each edge is given the value 1 or 0 depending on if it is on the shortest path or not.
...
// MAGIC %md
// MAGIC To make the code somewhat more accessible, we wrap the execution of the two parts above in a new function called `shortestPath`.
// MAGIC This new function takes the id of the start node and a single goal node as well as the input graph as input. 
// MAGIC The function then ouputs the path graph mentioned above.
...
// MAGIC %md
// MAGIC RDDs in Spark contain lineage graphs with information about previous operations, for e.g. fault tolerance. These can increase significantly in size after many transformations which may result in reduced performance, especially in iterative algorithms (such as in GraphX). For this reason, we truncate the lineage graph by checkpointing the RDDs.
...
// MAGIC 
// MAGIC In order to analys the data we visualize a graph which show the number times it has been visited by the exploration algorithm.
// MAGIC When visualised, the edges with multiple visits are marked with a thicker line. 
// MAGIC Note that the graph will get very cluttered if more than 50 nodes are in the graph.
// MAGIC 
// MAGIC But first, we need to initialize the d3 package.
...
// MAGIC %md
// MAGIC We can also visualize the shortest path in the graph.
...
// MAGIC %md
// MAGIC Now we visualize the instant regret. 
// MAGIC We should see a lot ot spikes in the beginning of the graph but the general trend should be that the curve decrease to zero while having fewer spikes as the algorithm gets closer to the optimal solutions. Note that there will always be some spikes as these corresponds to "exploratory" actions.
...
// MAGIC %md
// MAGIC We can also show the cumulative regret. As the algorithm reaches a final solution, the instantaneous regret should decrease and the cumulative regret should reach a plateau.
...
// MAGIC ## References
// MAGIC 
// MAGIC Sadikov, I., & Sainudiin, R.. (2016). Distributed Weighted Shortest Paths. <http://lamastex.org/lmse/mep/src/GraphXShortestWeightedPaths.html>.
// MAGIC 
// MAGIC Thompson, W. (1933). On the likelihood that one unknown probability exceeds another in view of the evidence of two samples. *Biometrika*, 25(3–4), 285-–294.
// MAGIC 
// MAGIC Wang, S., & Chen, W. (2018). Thompson sampling for combinatorial semi-bandits. In *International Conference on Machine Learning* (pp. 5114–5122).
