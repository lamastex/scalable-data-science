...
# MAGIC %md
# MAGIC ## UCluster on a single machine

# COMMAND ----------

# MAGIC %md
# MAGIC This code has been migrated to Tensorflow 2 from the original code, but no other changes have been made.
...
# MAGIC %md
# MAGIC Get the input parameters. Necessary modifications:
# MAGIC - import arguements by a namespace; not by an argument parser
# MAGIC - change the data directories appropriately
...
# MAGIC - calls the training one epoch
# MAGIC - calls the evaluation one epoch
# MAGIC - selects the loss and optimizer
...
# MAGIC - loop over all batches in the training (or evaluation) dataloader.
# MAGIC - prints training (and evaluation) loss.
# MAGIC - switches loss after n=10 epochs. Reason is that the full loss requires an initial guess for the position of the cluster centroids.
# MAGIC - prints training (and evaluation) cluster accuracy.
...
# MAGIC %md
# MAGIC Result: 
# MAGIC - We see that the learning on a single machine works.
# MAGIC - One epoch takes about 30 minutes
# MAGIC - Resulting accuracy lies at about 0.55