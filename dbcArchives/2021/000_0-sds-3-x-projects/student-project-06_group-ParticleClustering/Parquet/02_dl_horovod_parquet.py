...
# MAGIC %md 
# MAGIC This notebook extends the TF v2.x code from the single machine notebook `23_ds_single_machine`. The modification are such that the code enables multi-machine training using the [horovod](https://github.com/horovod/horovod) framwork. 
# MAGIC 
# MAGIC We will highlight the changes compared the single machine implementation.

# COMMAND ----------

# MAGIC %md
# MAGIC First: Check if the data is in local. If not, go to notebook `1_data_and_preprocssing` and download the data from dbfs to local.
...
# MAGIC %md
# MAGIC Get the input parameters.
...
# MAGIC %md
# MAGIC Define the utils functions.
...
# MAGIC %md
# MAGIC Modification:
# MAGIC - create checkpoint directory for horovod
# MAGIC - directory is user chosen
...
# MAGIC %md
# MAGIC Create horovod h5 loading function:
# MAGIC - note the rank and size is inputed.
# MAGIC - rank is the current device id
# MAGIC - size is the total number of available GPUs
# MAGIC - we split the data in the h5 file for each device. 
...
# MAGIC %md
# MAGIC Main training function. Modifications are:
# MAGIC - import packages again. Otherwise single devices may cause problems.
# MAGIC - initialise the horovod runner
# MAGIC - copy the files from local to each GPU such that they are available for horovod.
# MAGIC - scale the learning rate by the number of available devices.
# MAGIC - add a horovod specific distributed optimizer.
# MAGIC - use hooks for checkpoint saving ever 1000 steps.
# MAGIC - switch from a normal TF training session to a monitored training session.
...
# MAGIC %md
# MAGIC Training utils. Change this for reading from parquet.
...
# MAGIC %md
# MAGIC One epoch training and evaluation functions:
# MAGIC - the applicable horovod rank and size is fed into both functions.
# MAGIC - use the rank and size to load the correct h5 data.
# MAGIC - remove progress bars since progress bars from each device would overlap.
...
# MAGIC %md
# MAGIC Run the training:
# MAGIC - initialise the Horovod runner with np=2 GPUs. The cluster does not allow more GPUs
# MAGIC - run the horovod runner with the given training function.
...
# MAGIC %md
# MAGIC Results:
# MAGIC - Execution of the command for np=2 GPUs takes 3.39 hours.
# MAGIC - Plot below show the validation accuracy vs epoch.
# MAGIC - Note that we switch to the full loss after n=10 epochs.
# MAGIC - We observe an improvement in the cluster validation set accuracy after around 50 epochs.
# MAGIC - Highest cluster validation set accuracy lies at about 68%.
# MAGIC - Output of the algorithm is the stored model.

# COMMAND ----------

# MAGIC %md
# MAGIC ![The Standard Model](https://raw.githubusercontent.com/Tarnekar/bitstarter/master/validation_accuracy.png)
