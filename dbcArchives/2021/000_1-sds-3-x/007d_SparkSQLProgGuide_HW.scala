...
// MAGIC %md
// MAGIC ## Bucketing, Sorting and Partitioning
// MAGIC 
// MAGIC For file-based data source, it is also possible to bucket and sort or partition the output. Bucketing and sorting are applicable only to persistent tables:
...
// MAGIC - https://raw.githubusercontent.com/apache/spark/master/examples/src/main/scala/org/apache/spark/examples/sql/SQLDataSourceExample.scala 
// MAGIC in the Spark repo.
// MAGIC 
// MAGIC Note that partitioning can be used with both save and saveAsTable when using the Dataset APIs.
// MAGIC 
// MAGIC `partitionBy` creates a directory structure as described in the Partition Discovery section. 
// MAGIC Thus, it has limited applicability to columns with high cardinality. In contrast `bucketBy` distributes data across a fixed number of buckets and can be used when the number of unique values is unbounded. One can use `partitionBy` by itself or along with `bucketBy.
...
// MAGIC %md
// MAGIC We can also use a fixed number of buckets and sort by a column within each partition. Such finer control of the dataframe written as a parquet file can help with optimizing downstream operations on the dataframe.
