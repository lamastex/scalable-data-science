...
// MAGIC 
// MAGIC ### Avoid zip files!
// MAGIC 
// MAGIC Spark won't read zip files as it is proprietary format of compression. 
// MAGIC 
// MAGIC **So best practice is to first convert zip files to open compression formats like `gz` (GNU zip) or `bz2` files first and then load them into dbfs.**
