...
// MAGIC %md
// MAGIC # Tweet Streaming Collector - Track & Follow
// MAGIC 
// MAGIC In the previous notebook we were capturing tweets from the public streams under the assumption that it is a random sample of roughly 1% of all tweets.
// MAGIC 
// MAGIC In this notebook, we can modify the collector to focus on specific communications of interest to us. 
// MAGIC Specifically, by including:
// MAGIC 
// MAGIC - a list of strings to track and 
// MAGIC - a list of twitter user-IDs of interest to follow.
// MAGIC 
...
// MAGIC 
// MAGIC Next we import and instantiate for Twitter REST API, which allows us to obtain data from Twitter that is not in the live stream but in Twitter's storage layers containing archives of historcial events (including past status updates).
...
// MAGIC %md
// MAGIC ### Creating ScreenNames of Interest
...
// MAGIC %md
// MAGIC ### Extending `streamFunc` to Track & Follow
// MAGIC 
// MAGIC Now, let's extend our function to be able to track and follow.
...
// MAGIC %md
// MAGIC Collect for a few minutes so all fields are availabale in the Tweets... otherwise you will get errors like this  (which may be unavidable if what you are tracking has no `geoLocation` information for example, only a small fraction of Tweets have this information):
// MAGIC 
// MAGIC 
...
// MAGIC 
// MAGIC **NOTE** In certain experiments all the user-IDs being tracked may not have enabled `geoLocation` information. In such cases, you can use the `tweetsDF2TTTDFLightWeight` to obtain the TTTDF from the raw tweets.
