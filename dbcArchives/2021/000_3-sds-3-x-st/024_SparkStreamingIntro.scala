...
// MAGIC the input data stream received from source (for eg. Kafka, Flume, and Kinesis) or the processed data stream
...
// MAGIC 
// MAGIC ### Setting up a streaming source
...
// MAGIC 
// MAGIC First it is important to **stop existing StreamingContext if any** and then start/restart the new one.
// MAGIC 
...
// MAGIC %md 
// MAGIC 
// MAGIC Finally, if you want stop the StreamingContext, you can uncomment and execute the following code:
// MAGIC 
...
// MAGIC %md
// MAGIC Next two examples Spark Streaming is with live tweets.