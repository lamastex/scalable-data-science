``` md # Playing Games and Driving Cars: Reinforcement Learning
...
``` md # Principal Approaches: Deep Q-Learning and Policy Gradient Learning
...
```` md Next, let's look at the network itself -- it's super simple, so we can get that out of the way too:
...
```` md Note that the output layer has `num_actions` neurons.
