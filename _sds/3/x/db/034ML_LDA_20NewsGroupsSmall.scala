...
// MAGIC %md
// MAGIC ## Let's get an overview of LDA in Spark ML

// COMMAND ----------

// MAGIC %md
// MAGIC See [https://spark.apache.org/docs/latest/ml-clustering.html#latent-dirichlet-allocation-lda](https://spark.apache.org/docs/latest/ml-clustering.html#latent-dirichlet-allocation-lda)
...
// MAGIC 
// MAGIC We will now set the parameters for LDA. We will use setOptimizer("online") here, which implements Online Variational Bayes. This is actually the default optimizer but we do it explicitly here for clarity.
// MAGIC 
...
// MAGIC %md
// MAGIC We will set the parameters needed to build our LDA model. We can also setSubsamplingRate, which sets the fraction of corpus sampled and used at each iteration. In this example, we will set this to 0.8.
...
val topicIndices = ldaModel.describeTopics(maxTermsPerTopic = 5).as[(Int, Array[Int], Array[Double])]
topicIndices.show()
...
val topics = topicIndices.collect().map {
  case (topicnum, terms, termWeights) => terms.map(vocabList(_)).zip(termWeights)
}
...
topicIndices.select($"termIndices".as[Array[Int]]).show(5)

// COMMAND ----------

topicIndices.select($"termIndices".as[Array[Int]]).take(1)

// COMMAND ----------

topicIndices.select($"termIndices".as[Array[Int]]).take(1)(0)(0)

// COMMAND ----------

vocabList(topicIndices.select($"termIndices".as[Array[Int]]).take(1)(0)(0))
...
val new_lda = new LDA()
  .setK(numTopics)
  .setMaxIter(10) // more than 3 this time
  .setOptimizer("online")
  .setSubsamplingRate(0.8)
...
// MAGIC * Then find the `LDA` by scrolling below on the left to `clustering` methods and click on `LDA`
// MAGIC * Then find the `docConcentration` in the main window to the right and click it it expand. Read the description to find the default value.
// MAGIC 
// MAGIC From here you can also go directly to the source code for the LDA class by following the link near the top of the description for the LDA class. It will take you here: 
// MAGIC * [https://github.com/apache/spark/blob/v2.4.3/mllib/src/main/scala/org/apache/spark/ml/clustering/LDA.scala](https://github.com/apache/spark/blob/v2.4.3/mllib/src/main/scala/org/apache/spark/ml/clustering/LDA.scala)
...
val new_ldaModel = new_lda.fit(new_countVectors)

// COMMAND ----------

val topicIndices = new_ldaModel.describeTopics(maxTermsPerTopic = 5).as[(Int, Array[Int], Array[Double])]
val vocabList = vectorizer.vocabulary
val topics = topicIndices.collect().map {
  case (topicnum, terms, termWeights) => terms.map(vocabList(_)).zip(termWeights)
}
...
val em_ldaModel = em_lda.fit(new_countVectors)

// COMMAND ----------

val topicIndices = em_ldaModel.describeTopics(maxTermsPerTopic = 5).as[(Int, Array[Int], Array[Double])]
...
val vocabList = vectorizer.vocabulary
val topics = topicIndices.collect.map { 
  case (topicnum, terms, termWeights) => (topicnum, terms.map(vocabList(_)).zip(termWeights))
}
println(s"$numTopics topics:")
topics.foreach { case (i, topic) =>
  println(s"TOPIC $i")
  topic.foreach { case (term, weight) => println(s"$term\t$weight") }
  println(s"==========")
}
...
// MAGIC %md 
// MAGIC We've managed to get some good results here.
...
// Attach topic index to each term instead of to the entire topic and flatten.
val flatTopics = topics.flatMap {
  x: (Int, Array[(String, Double)]) => {
    val arrayOfTuple = x._2
    val topicId = x._1
...
// Turn the new topics array into an RDD
val termRDD = sc.parallelize(flatTopics)

// COMMAND ----------

// Create DF with proper column names
val termDF = termRDD.toDF.withColumnRenamed("_1", "term").withColumnRenamed("_2", "probability").withColumnRenamed("_3", "topicId")
...
// MAGIC 
// MAGIC 0. Try on Stanford Legal docs...
// MAGIC 
