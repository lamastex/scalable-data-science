...
/*
YouTry: Fix the expressions for grad_w1 and grad_w2 to have w_bc.value be the same as that from ml lib's fit.coefficients
*/    
for (i <- 1 to max_iter) {
        val grad_w1 = pts.map(x => -2*(w_bc.value(0)*x(0) + w_bc.value(1)*x(1) - x(2))*x(0)).reduce(_+_)
        val grad_w2 = pts.map(x => -2*(w_bc.value(0)*x(0) + w_bc.value(1)*x(1) - x(2))*x(1)).reduce(_+_)
        w_bc.value(0) = w_bc.value(0) - step*grad_w1
        w_bc.value(1) = w_bc.value(1) - step*grad_w2
}
