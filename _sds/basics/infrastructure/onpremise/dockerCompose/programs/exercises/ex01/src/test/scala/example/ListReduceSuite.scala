...
/**
 * This is an adaptation of the coursera spark+scala course;
 * You should really be taking this course and the three other Scala function 
 * programming and shared-maemory parallel programming courses in Scala that 
 * precede it! We are just in a hurry now, so this is just to get going.
 * To properly understand sbt, you need to start with `sbt by example` from:
 *  https://www.scala-sbt.org/1.0/docs/Combined+Pages.html
 *
 * This class implements a ScalaTest test suite for the methods in object
 * `ListReduce` that need to be implemented as part of this exercise. A test
 ...
 */
 @RunWith(classOf[JUnitRunner])
  class ListReduceSuite extends FunSuite {
  ...
  test("one plus one is three?") {
    assert(1 + 1 == 3) // This assertion fails! Please fix it.
  }
   ...
   *    [info]   2 did not equal 3 (ListReduceSuite.scala:67)
   * }}}
   *
   * We recommend to always use the `===` equality operator when writing tests.
   */
  test("details why one plus one is not three") {
    assert(1 + 1 === 3) // fix this!
  }
  ...
  /**
   * We only provide three very basic tests for you. Write more tests to make
   * sure your `sum`, `max` and `min` methods work as expected.
   *
