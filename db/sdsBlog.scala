...
// MAGIC #### AWS Educate credit pooling
// MAGIC 
// MAGIC We [shared these 21 steps](https://github.com/raazesh-sainudiin/scalable-data-science/blob/master/db/xtraResources/awsEducate/sharing.md) each student of the course could follow in order to share their AWS credit codes with us. These AWS credits were pooled into the databricks' AWS account that we managed for the course.
// MAGIC 
// MAGIC #### Setting some safe-guards in AWS
// MAGIC 
// MAGIC To prevent a student from accidentally launching hundreds of nodes we set some limits in the AWS account (including maximum number of nodes that can be launched and the sizes of the nodes, etc).
// MAGIC 
// MAGIC #### AWS Costs
// MAGIC 
// MAGIC We typically launched a 4 node cluster 24/7 on spot-prices.  This cluster only costed a maximum of 316 USD/month and averaged around 100 USD/month.  Thus we only needed credits from about 3 students per month to pay for this. Since several faculty sat in on the course and two of them also donated their AWS credits we were able to provide the 24/7 service for the duration of the course.
// MAGIC   
...
// MAGIC It also paid to look at historical spot-pricing time series from various AWS (sub)-zones to determine the currently cheapest places to launch our clusters on spot prices.
// MAGIC 
// MAGIC ### Databricks Academic Partners Program
// MAGIC 
// MAGIC #### Databricks Shard Settings
...
// MAGIC 
// MAGIC Students are trained to run their own big-data projects as part of their course requirements. This cutting-edge training using cloud infrastructure to solve big-data problems will generate globally competitive graduates for the data industry, Dr Sainudiin says.
// MAGIC 
// MAGIC With a curriculum created in consultation with the tech industry, the innovative course has received praise from Roger Jarquin, Chief Technical Officer of Wynyard Group. 
// MAGIC 
// MAGIC “Apache Spark is currently the fastest growing open source project. We at Wynyard rely on it for our successful products in advanced crime analytics. The syllabus of UC's course in scalable data science as well as the choice of the underlying technologies were designed with our R&D team's current needs in mind.
// MAGIC Wynyard’s data scientists and engineers are highly engaged with this course. 
// MAGIC Some of them have assited Raaz and Siva with the preparation and delivery of the course while some others are currently being trained in the course. 
// MAGIC We hope that such industry-academia collaborations will continue to be a dynamic training ground for future employees in our growing data industry,” says Jarquin, who is also an Adjunct Fellow of UC's School of Mathematics and Statistics.
// MAGIC 
// MAGIC Professor James Smithies, Director of King's Digital Lab, Department of Digital Humanities, King's College London, and former Senior Lecturer in History at UC, says the course in Scalable Data Science is “an excellent resource for the digital humanities, and sits very nicely beside activities occurring at King’s Digital Lab (KDL)”.
// MAGIC 
...
// MAGIC 
// MAGIC “While this Canterbury Tech outreach event is targeted at local industry, it is important to bring more awareness of the grant's infrastructure, potential benefits and utility to the UC community.”
// MAGIC 
// MAGIC Dr Sainudiin completed his PhD at Cornell University and a postdoctoral research fellowship at Oxford before joining UC in 2007.