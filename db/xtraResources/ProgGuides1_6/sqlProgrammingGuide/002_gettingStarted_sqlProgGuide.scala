...
// MAGIC ```
// MAGIC 
// MAGIC But in Databricks notebook (similar to `spark-shell`) `SQLContext` is already created for you and is available as `sqlContext`.
...
// MAGIC ```
// MAGIC org.apache.spark.sql.hive.HiveContext = org.apache.spark.sql.hive.HiveContext@7a955f82
// MAGIC ```
...
// MAGIC 
// MAGIC > Note that Hive installation is not required when working with SQLContext or HiveContext, Spark comes with built-in derby database.
...
// MAGIC 
// MAGIC > Note that performance for DataFrames is the same across languages Scala, Java, Python, and R. This is due to the fact that the planning phase is just language-specific, only logical plan is constructed in Python, and all the physical execution is compiled and executed as JVM bytecode.
...
// MAGIC 
// MAGIC We have already created a table of social media usage from NYC (you will see later how this table was built from raw data). 
// MAGIC > See the very bottom of this worksheet to see how this was done.
// MAGIC 
...
// MAGIC pythonDF = sqlContext.table("social_media_usage").select("platform").distinct()
// MAGIC pythonDF.show(3)

// COMMAND ----------

// MAGIC %r
// MAGIC ## Shift+Enter to use R in this cell to query "social_media_usage" table
// MAGIC rDF <- table(sqlContext, "social_media_usage")
// MAGIC selectedRDF <- distinct(select(rDF, "platform"))
// MAGIC 
// MAGIC showDF(selectedRDF, 3)
...
// MAGIC %md
// MAGIC > **Side-note:** Although, currently DataFrames and Datasets are separate classes in Spark, idiomatically DataFrame is just a special Dataset of a case class Row, and this is how it will be in the future releases of Spark > 1.6. Dataset API is considered experimental in Spark 1.6.
