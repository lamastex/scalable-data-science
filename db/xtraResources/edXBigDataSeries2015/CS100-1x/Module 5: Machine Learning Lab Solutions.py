# Databricks notebook source exported at Mon, 14 Mar 2016 03:39:24 UTC
# MAGIC %md
...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    movieRatings = IDandRatingsTuple[1]
    count = len(movieRatings)
    avgRating = sum(movieRatings) * 1.0 / count
    return (ID, (count, avgRating))
...
# PRIVATE_TEST Number of Ratings and Average Ratings for a Movie (1a)
...
# PRIVATE_TEST Movies with Highest Average Ratings (1b)
...
# PRIVATE_TEST Movies with Highest Average Ratings and more than 500 Reviews (1c)
Test.assertEquals(movieLimitedAndSortedByRatingRDD.count(), 194,
                'incorrect movieLimitedAndSortedByRatingRDD.count()')
Test.assertEquals(movieLimitedAndSortedByRatingRDD.take(20),
              [(4.5349264705882355, u'Shawshank Redemption, The (1994)', 1088),
...
def computeError(predictedRDD, actualRDD):
    ...
    # Using the total squared error and the number of ent, compute the RSME
...
# PRIVATE_TEST Root Mean Square Error (2b)
Test.assertTrue(abs(testError - 1.22474487139) < 0.00000001,
                'incorrect testError (expected 1.22474487139)')
Test.assertTrue(abs(testError2 - 3.16227766017) < 0.00000001,
                'incorrect testError2 result (expected 3.16227766017)')
...
validationForPredictRDD = validationRDD.map(lambda x: (x[0], x[1])).cache()
...
# PRIVATE_TEST Using ALS.train (2c)
tolerance = 0.03
Test.assertEquals(trainingRDD.getNumPartitions(), 2,
                  'incorrect number of partitions for trainingRDD (expected 2)')
Test.assertEquals(validationForPredictRDD.count(), 96902,
...
# PRIVATE_TEST Testing Your Model (2d)
tolerance = 0.03
Test.assertTrue(abs(testRMSE - 0.87809838344) < tolerance, 'incorrect testRMSE: %.11f' % testRMSE)
...
# PRIVATE_TEST Comparing Your Model (2e)
Test.assertTrue(abs(trainingAvgRating - 3.57409571052) < 0.000001,
                'incorrect trainingAvgRating (expected 3.57409571052): %.11f' % trainingAvgRating)
Test.assertTrue(abs(testAvgRMSE - 1.12036693569) < 0.000001,
                'incorrect testAvgRMSE (expected 1.12036693569): %.11f' % testAvgRMSE)
...
# ANSWER
How do your model's predicted ratings compare to using the average rating?
#[ ] The model is less accurate than using the average rating
#[ ] The model has comparable accuracy to using the average rating
#[x] The model is more accurate than using the average rating
...
print 'My movie ratings: %s' % myRatingsRDD.take(20)
...
myUnratedMoviesRDD = (moviesRDD
                      ...
                      .filter(lambda x: x not in myRatedMovies))
