...
4. Send an Email to your instructor's `math.uu.se` email address from your `uu.se` email address as follows:
  ...
  - In the `Subject:` field of the email write `1MS035:Assignment0x` where `0x` is the number of the assignment
...
1. Weeks 1--3: 
  ...
  - Give a *proof of the LLN* that was required in the complete proof of the claim that *(asymptotic) consistency of the sample mean estimator for the product Bernoulli experiment "follows" from the (Weak) Law of Large Numbers or LLN*.
